...
describe('ai module', () => {
  ...
  describe('validateSuggestResult', () => {
    it('should return valid for correct schema', () => {
      const result = {
        suggested_response_id: 'needinfo',
        draft_response: 'Thank you for the report. Could you provide more details?',
        reasoning: 'Bug lacks clear steps to reproduce',
      };
      const validation = validateSuggestResult(result);
      expect(validation.valid).toBe(true);
    });

    it('should return invalid when suggested_response_id is missing', () => {
      const result = {
        draft_response: 'Some response',
      };
      const validation = validateSuggestResult(result);
      expect(validation.valid).toBe(false);
      expect(validation.errors).toContain('suggested_response_id must be a string');
    });

    it('should return invalid when draft_response is missing', () => {
      const result = {
        suggested_response_id: 'needinfo',
      };
      const validation = validateSuggestResult(result);
      expect(validation.valid).toBe(false);
      expect(validation.errors).toContain('draft_response must be a string');
    });

    it('should allow optional reasoning', () => {
      const result = {
        suggested_response_id: 'needinfo',
        draft_response: 'Thank you for the report.',
      };
      const validation = validateSuggestResult(result);
      expect(validation.valid).toBe(true);
    });

    it('should validate reasoning type if provided', () => {
      const result = {
        suggested_response_id: 'needinfo',
        draft_response: 'Thank you for the report.',
      ...
      const validation = validateSuggestResult(result);
      expect(validation.valid).toBe(false);
      expect(validation.errors).toContain('reasoning must be a string if provided');
    });
  });
  ...
  describe('suggestCannedResponse', () => {
    it('should return empty suggestion when provider not configured', async () => {
      const result = await suggestCannedResponse(
        { id: 123 },
        [{ id: 'test' }],
        { provider: 'none' }
      );
      expect(result.suggested_response_id).toBe('');
      expect(result.draft_response).toBe('');
  ...
  describe('backend proxy mode', () => {
    it('should call backend proxy for backend transport', async () => {
      ...
      const testBackendUrl = 'http://my-backend.example.com:8080';
      const result = await classifyBug(
        { id: 123, summary: 'Test' },
        { provider: 'claude', transport: 'backend', backendUrl: testBackendUrl }
      );

      expect(global.fetch).toHaveBeenCalledWith(
        `${testBackendUrl}/api/ai/classify`,
        expect.objectContaining({
          method: 'POST',
        })
      );
    ...
    it('should use relative path when backendUrl is empty', async () => {
      const mockBackendResponse = {
        ai_detected_str: false,
        ai_detected_test_attached: false,
        crashstack_present: false,
      ...
      await classifyBug(
        { id: 123, summary: 'Test' },
        { provider: 'claude', transport: 'backend', backendUrl: '' }
      );
  ...
  describe('generateResponse', () => {
    ...
    it('should use backend proxy when configured', async () => {
      ...
      const testBackendUrl = 'https://api.example.org';
      const result = await generateResponse(
        sampleBug,
        { mode: 'response' },
        { provider: 'claude', transport: 'backend', backendUrl: testBackendUrl }
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        `${testBackendUrl}/api/ai/generate`,
        expect.any(Object)
      );
  ...
  describe('refineResponse', () => {
    ...
    it('should use backend proxy when configured', async () => {
      ...
      const testBackendUrl = 'http://backend.local:9000';
      const result = await refineResponse(
        sampleBug,
        currentResponse,
        userInstruction,
