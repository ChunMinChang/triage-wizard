...
import {
  ...
  getSelectedRefineChips,
  clearSelectedRefineChips,
} from '../ui.js';
...
describe('ui module', () => {
  ...
  describe('refine chips', () => {
    beforeEach(() => {
      // Add refine chips to the DOM
      const chipsContainer = document.createElement('div');
      chipsContainer.className = 'refine-chips';
    ...
    describe('getSelectedRefineChips', () => {
      it('should return empty array when no chips are selected', () => {
        const selected = getSelectedRefineChips();
        expect(selected).toEqual([]);
      });

      it('should return instructions from selected chips', () => {
        const chips = document.querySelectorAll('.refine-chips .chip');
        chips[0].classList.add('selected');
        chips[2].classList.add('selected');

        const selected = getSelectedRefineChips();
        expect(selected).toEqual(['Make it shorter', 'Add request for STR']);
      });

      it('should return all instructions when all chips are selected', () => {
        const chips = document.querySelectorAll('.refine-chips .chip');
        chips.forEach(chip => chip.classList.add('selected'));

        const selected = getSelectedRefineChips();
        expect(selected).toHaveLength(3);
        expect(selected).toContain('Make it shorter');
        expect(selected).toContain('Use a friendlier tone');
        expect(selected).toContain('Add request for STR');
      });
    });

    describe('clearSelectedRefineChips', () => {
      it('should remove selected class from all chips', () => {
        const chips = document.querySelectorAll('.refine-chips .chip');
        chips.forEach(chip => chip.classList.add('selected'));

        clearSelectedRefineChips();

        const selectedAfter = document.querySelectorAll('.refine-chips .chip.selected');
        expect(selectedAfter).toHaveLength(0);
      });

      it('should do nothing if no chips are selected', () => {
        clearSelectedRefineChips();

        const chips = document.querySelectorAll('.refine-chips .chip');
        expect(chips).toHaveLength(3); // chips still exist
        chips.forEach(chip => {
